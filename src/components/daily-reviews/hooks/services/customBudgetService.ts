  ...
  try {
    ...
    if (error) {
      ...
      if (platform === 'meta') {
        ...
        console.log(`[customBudgetService] Retornando dados legados: ${JSON.stringify(legacyData?.[0])}`);
    ...
    console.log(`[customBudgetService] Encontrados ${data.length} orçamentos personalizados`);
    ...
    console.log(`[customBudgetService] Retornando primeiro orçamento: ${JSON.stringify(data[0])}`);
    return data[0] || null;
  } catch (err) {
    console.error("[customBudgetService] Erro inesperado ao buscar orçamento personalizado:", err);
    return null;
  }
}
...
export function prepareCustomBudgetInfo(customBudget: any | null) {
  ...
  console.log(`[customBudgetService] Preparando informações do orçamento personalizado: ${JSON.stringify(customBudget)}`);
