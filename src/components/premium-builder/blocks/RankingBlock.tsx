...
const ICONS: Record<RankingDataSource, React.ComponentType<{ className?: string; style?: React.CSSProperties }>> = {
  regions: MapPin, campaigns: BarChart3, creatives: ImageIcon, age: Users, gender: UserCircle2,
};

const TITLES: Record<RankingDataSource, string> = {
  regions: 'Top Regiões', campaigns: 'Top Campanhas', creatives: 'Top Criativos',
  age: 'Top Faixas Etárias', gender: 'Por Gênero',
};

const SUBTITLES: Record<RankingDataSource, string> = {
  regions: 'Localidades por performance', campaigns: 'Campanhas com melhor desempenho',
  creatives: 'Anúncios destaque', age: 'Distribuição por idade', gender: 'Distribuição por gênero',
};

function buildItems(
  source: RankingDataSource, metric: MetricKey,
  demographics?: any, campaigns?: any[], topAds?: any[],
  ...
  if (source === 'age') {
    return (demographics?.age || []).map((r: any) => ({ label: r.range || r.label || '—', value: r[metric] ?? r.value ?? 0 }));
  }
  if (source === 'gender') {
    return (demographics?.gender || []).map((r: any) => ({ label: r.gender || r.label || '—', value: r[metric] ?? r.value ?? 0 }));
  }
  if (source === 'campaigns') {
...
export const RankingBlock: PremiumBlockRenderer = ({ block, data }) => {
  const cfg = block.config;
  const source = cfg.rankingSource || 'regions';
  const metric = cfg.metric || 'conversions';
  const accent = cfg.accent || '#ff6e00';
  const limit = cfg.limit || 8;
  const Icon = ICONS[source];
  const title = cfg.title || TITLES[source];

  const items = buildItems(source, metric, data.demographics, data.campaigns, data.topAds)
    .filter((i) => i.value > 0)
    ...
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-white/[0.03] to-transparent p-6 backdrop-blur-xl h-full">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-[11px] text-white/40">{SUBTITLES[source]} · {METRIC_LABEL[metric]}</p>
        </div>
      </div>
