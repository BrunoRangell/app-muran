        ...
        <div className="space-y-4">
          {clientDates.map((date, index) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center justify-center h-14 w-14 rounded-full shrink-0 transition-all duration-300 ${
                  isToday 
                    ? 'bg-white/20 ring-2 ring-white/40' 
                    : isTomorrow
                    ? 'bg-blue-100 ring-2 ring-blue-300'
                    : 'bg-muran-primary/10 ring-2 ring-muran-primary/20 group-hover:ring-muran-primary/40'
                }`}>
                  {date.type === 'partnership_anniversary' ? (
                    <Handshake size={24} className={isToday ? 'text-white' : isTomorrow ? 'text-blue-600' : 'text-muran-primary'} />
                  ) : (
                    <Cake size={24} className={isToday ? 'text-white' : isTomorrow ? 'text-blue-600' : 'text-muran-primary'} />
                  )}
                </div>
