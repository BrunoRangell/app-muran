...
export function MetaAdsTabV2({ onRefreshCompleted }: MetaAdsTabProps) {
  const { toast } = useToast();
  const [modalClientId, setModalClientId] = useState<string | null>(null);

  // Buscar clientes com contas de Meta Ads
  const {
    data: clients,
    isLoading: isLoadingClients,
    refetch,
  } = useQuery({
    queryKey: ["clients-with-reviews-meta-v2"],
    queryFn: async () => {
      // Consultar clientes ativos com contas Meta Ads
      const { data, error } = await supabase
        .from("clients")
        .select("*, meta_accounts:client_meta_accounts(*), lastReview:daily_budget_reviews(*)")
        .eq("status", "active")
      ...
      return data.map((client) => {
        ...
        const sortedReviews = client.lastReview
          ? [...client.lastReview].sort(
