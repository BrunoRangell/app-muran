...
interface PremiumKpiWidgetProps {
  metric: MetricKey;
...
export function PremiumKpiWidget({
  metric,
  data,
  accent = '#ff6e00',
  showComparison = true,
  title,
}: PremiumKpiWidgetProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative space-y-2">
        ...
        <p className="text-2xl font-bold tracking-tight text-white">{formatValue(metric, current)}</p>
        ...
        <div className="pt-2">
          ...
          {showComparison && (
            <p className="mt-2 text-[10px] text-white/35">
              Anterior: <span className="text-white/55">{formatValue(metric, previous)}</span>
            </p>
          )}
        </div>
      </div>
