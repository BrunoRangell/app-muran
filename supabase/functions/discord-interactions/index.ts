...
async function handleOtimizacaoCommand(
  appId: string,
  token: string,
  nome: string,
  supabase: ReturnType<typeof createClient>,
  ...
  try {
    // Buscar clientes ativos com nome parecido
    const { data: matches, error } = await supabase
      .from('clients')
      .select('id, company_name')
      .eq('status', 'active')
      .ilike('company_name', `%${nome}%`)
      .limit(10);
    ...
    if (!matches || matches.length === 0) {
      await editOriginal(appId, token, {
        content: `❌ Nenhum cliente ativo encontrado para **"${nome}"**.`,
      });
      return;
    }

    if (matches.length > 1) {
      const list = matches.map((c: any) => `• ${c.company_name}`).join('\n');
      await editOriginal(appId, token, {
        content:
          `Encontrei **${matches.length}** clientes parecidos com "${nome}". ` +
          `Seja mais específico:\n${list}`,
      });
      return;
    }

    const client = matches[0] as any;
    ...
    if (invokeError) {
      console.error('[otimizacao] invoke error', invokeError);
      await editOriginal(appId, token, {
        content: `⚠️ Erro ao iniciar análise: ${invokeError.message || 'falha desconhecida'}`,
      });
    }
  } catch (e: any) {
    console.error('[otimizacao] erro', e);
    await editOriginal(appId, token, {
      content: `⚠️ Erro: ${e?.message || 'falha desconhecida'}`,
    });
  }
